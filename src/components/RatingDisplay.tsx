...
function gradeColorClass(isPassing: boolean): string {
  return isPassing
    ? 'text-[var(--tg-success)]'
...
function RatingDisplayInner({ summary }: RatingDisplayProps) {
  ...
  const outcomeGrade = useMemo(
    () => ratingToGradeInfo(displayValue, summary.disciplineType),
    [displayValue, summary.disciplineType],
  )

  const showOutcomeGrade =
    summary.finalRating !== null ||
    (summary.canSkipExam && summary.waivedRating !== null) ||
    summary.preliminaryRating !== summary.currentRating

  return (
    <Card variant="hero" className="p-4 sm:p-5">
      <div className="flex items-start gap-4">
        <RatingGauge value={summary.currentRating} label="R_тек" />
        <div className="min-w-0 flex-1">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="stat-label">R_тек</p>
              <p className="stat-value mt-1 text-2xl sm:text-[1.75rem]">
                <CountUp value={summary.currentRating} suffix=" %" />
              </p>
              ...
              <p
                className={`mt-1.5 text-sm font-semibold capitalize ${gradeColorClass(summary.currentGradeInfo.isPassing)}`}
              >
                {summary.currentGradeLabel}
              </p>
            </div>
      ...
      <div className="mt-4 rounded-[calc(var(--tg-radius)-0.25rem)] bg-[var(--tg-accent-soft)] px-4 py-4 text-center">
        ...
        <p className="stat-value mt-1 text-3xl text-[var(--tg-accent)] sm:text-4xl">
          <CountUp value={displayValue} suffix=" %" />
        </p>
        <p className="mt-1 text-xs text-[var(--tg-hint)]">{displayHint}</p>
        {showOutcomeGrade && (
          <p
            className={`mt-2 text-sm font-semibold capitalize ${gradeColorClass(outcomeGrade.isPassing)}`}
          >
            {formatGradeInfo(outcomeGrade)}
          </p>
        )}
      </div>

      <div className="mt-4 border-t border-[var(--tg-separator)] pt-4">
        <p className="mb-2 stat-label">Шкала оценок</p>
        <GradeScale rating={displayValue} disciplineType={summary.disciplineType} />
      </div>
    </Card>
  )
}
